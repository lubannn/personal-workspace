...
CalendarEvent 可以通过 `linked_entity_type = task` 和 `linked_entity_id` 引用 Task，但事件写入是独立 Git 提交，绝不改写 Task 的 DDL、状态或耗时。开放导出会验证引用的 Task 同包存在；提醒、重复、全天、编辑/取消和外部同步在各自规则完成前不开放。
