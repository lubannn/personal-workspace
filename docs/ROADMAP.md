...
当前切片已覆盖 Learning Area、Habit v1、健康指标暂存，以及手工睡眠的夜间睡眠/小睡/未确定分类、人工更正和原子确认入库。睡眠归属日固定为开始日。基于正式夜间睡眠的早睡/早起规则可生成带证据和解释的建议，但只有用户确认后才写入 HabitCheckIn；COROS 接入和无人确认的自动打卡仍保持关闭。
