...
Legacy Word 已提供 local-only `.docx` Preview、可审计单段修正、可比较重解析和 Dry Run staging ZIP：浏览器内计算源哈希、读取 OOXML 段落、生成按日 Markdown 与 diagnostics，明确展示 orphan/不支持对象，并将修正链、输出哈希和不含正文的 Import Log 放入下载包。正式写入前 plan 还能按最多 25 天的小批次确定性构造 segment-mode Entry/Revision/Segment，识别精确重试与冲突，并生成 checkpoint/软删除回滚 preview；生产 Commit 仍固定关闭。该流程不写 Journal、GitHub 或 Obsidian，也没有使用真实日记原件；完整边界见 `JOURNAL_IMPORT_SPEC.md` 和 `PHASE_3_LEGACY_COMMIT_PLAN.md`。
