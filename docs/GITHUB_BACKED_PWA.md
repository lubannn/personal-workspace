...
2026-08-28 正式环境验收通过：canonical Private 仓库的 16 个文件（`workspace.json` + 15 条 Capture）全部判定为 current，migratable 0、blocked 0；页面错误与警告日志为空，验收过程中没有 GitHub 写入。

## 12. Dashboard layout

Dashboard 布局保存在 `config/dashboard-layout.json`，不进入 LocalStorage。文件记录 owner、布局版本、Widget 顺序、显隐、尺寸、隐私模式和可扩展 settings；读取时保存 Git blob SHA，更新时提交该 SHA，跨设备陈旧写入会显式冲突。
