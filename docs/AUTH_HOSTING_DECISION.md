...
现有 `lubannn.github.io/personal-workspace/` 保持在线，继续作为手工 PAT 回退入口。
...
1. 一个 Worker：`personal-workspace`
2. 一组与 Worker 同部署的静态 PWA assets
...
4. 先部署不含 secret、不连接 D1 的静态壳与 `/health`；在四类设备和实际网络中验证可用性。
