...
function InspectionResult({ result }: { result: PortabilityResult }) {
  return (
    <div className={`portability-result ${result.valid ? "valid" : "invalid"}`} role="status">
      ...
      <p>{result.files} 个文件 · {result.captures} 条 Capture · {result.tasks} 条 Task · {result.timeEntries} 条 Time Entry · {result.projects} 个 Project · {result.projectPhases} 个阶段 · {result.milestones} 个里程碑 · {result.projectNotes} 条 Project Note · {result.projectFileReferences} 条文件引用 · {result.activityEvents} 条 Activity · {result.calendarEvents} 条 Calendar Event · {result.reportDrafts} 份 ReportDraft · {result.journalEntries} 篇 Journal · {result.journalSegments} 个 Journal Segment · {result.journalRevisions} 个 Journal Revision · {result.journalImportCheckpoints} 个导入 Checkpoint · {result.obsidianDocuments} 个 Obsidian 基线 · {result.syncConflicts} 个同步冲突 · {result.learningAreas} 个学习领域 · {result.learningGoals} 个学习目标 · {result.learningActivities} 条学习活动 · {result.learningResources} 个学习资源 · {result.habits} 个习惯 · {result.habitRules} 个习惯规则 · {result.habitCheckIns} 条打卡 · {result.healthStagingRecords} 条健康暂存 · {result.healthMetrics} 条健康指标 · {result.sleepSessions} 段睡眠 · {result.dashboardLayouts} 个 Dashboard 布局</p>
    ...
    <section className="portability-card" aria-labelledby="portability-title">
      ...
      <div className="portability-grid">
        <article>
          ...
          <p>读取 workspace.json、Dashboard 布局和全部 canonical collection（含 Journal Segment/Revision、导入 Checkpoint 与 ReportDraft 事实快照），生成带 SHA-256、Git blob SHA、文件数量与 schema 版本的 JSON。</p>
            ...
            : !connection ? <p className="restore-gate">先连接来源 Private 仓库，复用当前页面内存中的临时授权检查目标。</p>
              : <>
                ...
                {restorePlan ? <div className={`restore-plan ${restorePlan.ready ? "valid" : "invalid"}`} role="status">
                  ...
                  <p>{restorePlan.counts.files} 个文件 · {restorePlan.counts.captures} 条 Capture · {restorePlan.counts.tasks} 条 Task · {restorePlan.counts.timeEntries} 条 Time Entry · {restorePlan.counts.projects} 个 Project · {restorePlan.counts.projectPhases} 个阶段 · {restorePlan.counts.milestones} 个里程碑 · {restorePlan.counts.projectNotes} 条 Project Note · {restorePlan.counts.projectFileReferences} 条文件引用 · {restorePlan.counts.activityEvents} 条 Activity · {restorePlan.counts.calendarEvents} 条 Calendar Event · {restorePlan.counts.reportDrafts} 份 ReportDraft · {restorePlan.counts.journalEntries} 篇 Journal · {restorePlan.counts.journalSegments} 个 Segment · {restorePlan.counts.journalRevisions} 个 Revision · {restorePlan.counts.journalImportCheckpoints} 个导入 Checkpoint · {restorePlan.counts.obsidianDocuments} 个 Obsidian 基线 · {restorePlan.counts.syncConflicts} 个同步冲突 · {restorePlan.counts.learningAreas} 个学习领域 · {restorePlan.counts.learningGoals} 个学习目标 · {restorePlan.counts.learningActivities} 条学习活动 · {restorePlan.counts.learningResources} 个学习资源 · {restorePlan.counts.habits} 个习惯 · {restorePlan.counts.habitRules} 个习惯规则 · {restorePlan.counts.habitCheckIns} 条打卡 · {restorePlan.counts.healthStagingRecords} 条健康暂存 · {restorePlan.counts.healthMetrics} 条健康指标 · {restorePlan.counts.sleepSessions} 段睡眠 · 单个原子 commit</p>
