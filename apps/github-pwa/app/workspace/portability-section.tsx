...
function InspectionResult({ result }: { result: PortabilityResult }) {
  return (
    <div className={`portability-result ${result.valid ? "valid" : "invalid"}`} role="status">
      ...
      <p>{result.files} 个文件 · {result.captures} 条 Capture · {result.tasks} 条 Task · {result.projects} 个 Project · {result.projectPhases} 个阶段 · {result.milestones} 个里程碑 · {result.projectNotes} 条 Project Note · {result.activityEvents} 条 Activity · {result.calendarEvents} 条 Calendar Event · {result.dashboardLayouts} 个 Dashboard 布局</p>
    ...
    <section className="portability-card" aria-labelledby="portability-title">
      ...
      <div className="portability-grid">
        <article>
          ...
          <p>读取 workspace.json、Dashboard 布局、全部 Capture、Task、Project、阶段、里程碑、Project Note、Activity 和 Calendar Event，生成带 SHA-256、Git blob SHA、文件数量与 schema 版本的 JSON。</p>
