...
function InspectionResult({ result }: { result: PortabilityResult }) {
  return (
    <div className={`portability-result ${result.valid ? "valid" : "invalid"}`} role="status">
      ...
      <p>{result.files} 个文件 · {result.captures} 条 Capture · {result.tasks} 条 Task · {result.projects} 个 Project · {result.projectPhases} 个阶段 · {result.milestones} 个里程碑 · {result.dashboardLayouts} 个 Dashboard 布局</p>
    ...
    <section className="portability-card" aria-labelledby="portability-title">
      ...
      <div className="portability-grid">
        <article>
          ...
          <p>读取 workspace.json、Dashboard 布局、全部 Capture、Task、Project、阶段和里程碑，生成带 SHA-256、Git blob SHA、文件数量与 schema 版本的 JSON。</p>
            ...
            : !connection ? <p className="restore-gate">先连接来源 Private 仓库，复用当前页面内存中的临时授权检查目标。</p>
              : <>
                ...
                {restorePlan ? <div className={`restore-plan ${restorePlan.ready ? "valid" : "invalid"}`} role="status">
                  ...
                  <p>{restorePlan.counts.files} 个文件 · {restorePlan.counts.captures} 条 Capture · {restorePlan.counts.tasks} 条 Task · {restorePlan.counts.projects} 个 Project · {restorePlan.counts.projectPhases} 个阶段 · {restorePlan.counts.milestones} 个里程碑 · 单个原子 commit</p>
