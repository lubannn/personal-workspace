...
export function CorosFilePreflightSection({ timezone = "Asia/Shanghai" }: { timezone?: string }) {
  ...
  return <div className="legacy-import" aria-labelledby="coros-file-preflight-title">
    ...
    {preview ? <>
      ...
      <div className={`legacy-import-gate ${preview.readyForMapping ? "ready" : "blocked"}`} role="status"><strong>{preview.readyForMapping ? "结构与映射预检通过" : "结构预检被阻断"}</strong><p>{preview.readyForMapping ? "已生成确定性 Workout 候选与 staging envelope 计划；协议仍未激活，不会写入 staging 或 canonical 记录。" : "文件缺少建立稳定活动身份所需的结构或时间信息。"}</p></div>
      ...
      <div className="legacy-import-source"><div><strong>Workout staging envelope</strong><span>plan v{preview.stagingPlan.planVersion} · 协议未激活</span></div><small>{preview.stagingPlan.protocolDecision.reason}</small><small>非重复计划 {preview.stagingPlan.items.length} 条 · 跳过重复 {preview.stagingPlan.skippedDuplicateCount} 条</small></div>
