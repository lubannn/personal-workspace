...
export function HealthStagingSection({ connection, adapter, online, todayDate, staging, metrics, sleepSessions, workouts, loading, saving, savingId, onCreate, onCorrect, onCreateSleep, onCorrectSleep, onConfirm, onReject, onRefresh, onStaged, onBatchConfirmed }: Props) {
  ...
  function beginEdit(item: SyncedHealthStagingRecord) {
    if (item.record.data.health_type === "workout") return;
  ...
  return <section className="learning-card health-card" aria-labelledby="health-title">
    ...
    <CorosFilePreflightSection timezone={connection?.timezone ?? "Asia/Shanghai"} adapter={adapter} ownerId={connection?.ownerId ?? null} online={online} stagingRecords={staging} onStaged={onStaged} onBatchConfirmed={onBatchConfirmed} onRefresh={onRefresh} />
  </section>;
}
