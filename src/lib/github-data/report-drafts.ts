...
export type ReportDraftSourceSnapshot = {
  entity_type: "task" | "milestone" | "calendar_event" | "activity_event" | "project";
  id: string;
  version: number;
  title: string;
...
export type ReportDraftData = {
  ...
  scope_json: {
    ...
    source_entity_types: ["task", "project", "milestone", "calendar_event", "activity_event"];
  };
  facts_snapshot_json: {
...
const SOURCE_TYPES = new Set(["task", "milestone", "calendar_event", "activity_event", "project"]);
...
function isValidData(data: Record<string, unknown>): data is ReportDraftData {
  ...
  return (data.report_type === "weekly" || data.report_type === "monthly")
    ...
    && data.scope_json.source_entity_types.join(",") === "task,project,milestone,calendar_event,activity_event"
