...
const timeEntry = (id: string, localDate: string, durationMinutes: number, deletedAt: string | null = null): TimeEntryRecord => ({ ...base, deleted_at: deletedAt, entity_type: "time_entry", id, data: { task_id: "task_in", project_id: "project_1", local_date: localDate, timezone: "Asia/Shanghai", started_at: null, ended_at: null, duration_minutes: durationMinutes, entry_method: "manual_duration", notes_markdown: "", source_ref: null } });
...
describe("deterministic reports", () => {
  it("builds a timezone-aware Monday-to-Sunday report with traceable facts", () => {
    const report = buildDeterministicReport({
      ...
      timeEntries: [timeEntry("time_entry_in", "2026-09-02", 75), timeEntry("time_entry_out", "2026-08-30", 20), timeEntry("time_entry_deleted", "2026-09-03", 30, "2026-09-04T00:00:00.000Z")],
    });
    ...
    expect(report.timeEntries.map((record) => record.id)).toEqual(["time_entry_in"]);
    expect(report.trackedMinutes).toBe(75);
  ...
  it("exports stable CSV rows and neutralizes spreadsheet formulas", () => {
    const report = buildDeterministicReport({ reportType: "monthly", anchorDate: "2026-09-15", timezone: "Asia/Shanghai", tasks: [task("task_formula", "=SUM(1,1)", "2026-09-01T01:00:00.000Z")], projects: [project], milestones: [], calendarEvents: [], activityEvents: [], timeEntries: [timeEntry("time_entry_csv", "2026-09-02", 60)] });
    ...
    expect(csv).toContain('"time_entry","time_entry","time_entry_csv","data/time-entries/time_entry_csv.json"');
  });
  ...
  it("renders a traceable personal Markdown draft and escapes headings in facts", () => {
    const report = buildDeterministicReport({ reportType: "weekly", anchorDate: "2026-09-02", timezone: "Asia/Shanghai", tasks: [task("task_markdown", "*发布* [v1]", "2026-09-01T01:00:00.000Z")], projects: [project], milestones: [milestone], calendarEvents: [calendarEvent], activityEvents: [activity], timeEntries: [timeEntry("time_entry_markdown", "2026-09-02", 75)] });
    ...
    expect(markdown).toContain("Time Entry：1 条 / 1 小时 15 分钟");
    expect(markdown).toContain("`time_entry:time_entry_markdown`");
