...
import { isWorkoutLinkedToStaging, parseWorkoutRecord } from "./workouts";
...
export async function inspectPortableWorkspaceExport(value: unknown): Promise<ExportInspection> {
  ...
  const learningAreaRecords = new Map<string, ReturnType<typeof parseLearningAreaRecord>>();
  ...
  for (const file of learningAreaFiles) {
    try {
      ...
      if (learningAreaRecords.has(record.id)) errors.push({ code: "DUPLICATE_LEARNING_AREA_ID", message: "导出包中存在重复 LearningArea ID。", path: file.path });
      learningAreaRecords.set(record.id, record);
    } catch {
  ...
  const learningGoalRecords = new Map<string, ReturnType<typeof parseLearningGoalRecord>>();
  const learningGoalFiles = validPayloadFiles.filter((file) => file.path.startsWith("data/learning-goals/"));
  result.counts.learningGoals = learningGoalFiles.length;
  for (const file of learningGoalFiles) {
    try {
      const record = parseLearningGoalRecord(file.content);
      if (result.workspace && record.owner_id !== result.workspace.owner_id) errors.push({ code: "OWNER_MISMATCH", message: "LearningGoal 的 owner_id 与 workspace 不一致。", path: file.path });
      if (recordPath("learning_goal", record.id) !== file.path) errors.push({ code: "LEARNING_GOAL_PATH_MISMATCH", message: "LearningGoal 的 ID 与文件路径不一致。", path: file.path });
      if (learningGoalRecords.has(record.id)) errors.push({ code: "DUPLICATE_LEARNING_GOAL_ID", message: "导出包中存在重复 LearningGoal ID。", path: file.path });
      learningGoalRecords.set(record.id, record);
      const area = learningAreaRecords.get(record.data.learning_area_id);
      if (!area) {
        errors.push({ code: "LEARNING_GOAL_AREA_MISSING", message: "LearningGoal 引用的 LearningArea 不在导出包中或无法通过校验。", path: file.path });
      } else {
        if (area.owner_id !== record.owner_id) errors.push({ code: "LEARNING_GOAL_AREA_OWNER_MISMATCH", message: "LearningGoal 与引用的 LearningArea owner 不一致。", path: file.path });
        if (area.deleted_at !== null || area.data.status !== "active") warnings.push({ code: "LEARNING_GOAL_AREA_READ_ONLY", message: "LearningGoal 引用的 LearningArea 当前不可管理；Goal 保留并在恢复 Area 前只读。", path: file.path });
      }
    } catch {
      errors.push({ code: "INVALID_LEARNING_GOAL_RECORD", message: "LearningGoal 文件无法通过结构校验。", path: file.path });
    }
  }
  const rawLearningGoalCount = manifestCounts?.learning_goals;
  if ((rawLearningGoalCount !== undefined || learningGoalFiles.length > 0) && rawLearningGoalCount !== learningGoalFiles.length) errors.push({ code: "LEARNING_GOAL_COUNT_MISMATCH", message: "LearningGoal 数量与 manifest 不一致。" });

  const learningActivityIds = new Set<string>();
  const learningActivityFiles = validPayloadFiles.filter((file) => file.path.startsWith("data/learning-activities/"));
  result.counts.learningActivities = learningActivityFiles.length;
  for (const file of learningActivityFiles) {
    try {
      const record = parseLearningActivityRecord(file.content);
      if (result.workspace && record.owner_id !== result.workspace.owner_id) errors.push({ code: "OWNER_MISMATCH", message: "LearningActivity 的 owner_id 与 workspace 不一致。", path: file.path });
      if (recordPath("learning_activity", record.id) !== file.path) errors.push({ code: "LEARNING_ACTIVITY_PATH_MISMATCH", message: "LearningActivity 的 ID 与文件路径不一致。", path: file.path });
      if (learningActivityIds.has(record.id)) errors.push({ code: "DUPLICATE_LEARNING_ACTIVITY_ID", message: "导出包中存在重复 LearningActivity ID。", path: file.path });
      learningActivityIds.add(record.id);
      const area = learningAreaRecords.get(record.data.learning_area_id);
      if (!area) errors.push({ code: "LEARNING_ACTIVITY_AREA_MISSING", message: "LearningActivity 引用的 LearningArea 不在导出包中或无法通过校验。", path: file.path });
      else {
        if (area.owner_id !== record.owner_id) errors.push({ code: "LEARNING_ACTIVITY_AREA_OWNER_MISMATCH", message: "LearningActivity 与引用的 LearningArea owner 不一致。", path: file.path });
        if (area.deleted_at !== null || area.data.status !== "active") warnings.push({ code: "LEARNING_ACTIVITY_PARENT_READ_ONLY", message: "LearningActivity 的 Area 当前不可管理；历史记录保留并只读。", path: file.path });
      }
      if (record.data.goal_id) {
        const goal = learningGoalRecords.get(record.data.goal_id);
        if (!goal) errors.push({ code: "LEARNING_ACTIVITY_GOAL_MISSING", message: "LearningActivity 引用的 LearningGoal 不在导出包中或无法通过校验。", path: file.path });
        else {
          if (goal.owner_id !== record.owner_id) errors.push({ code: "LEARNING_ACTIVITY_GOAL_OWNER_MISMATCH", message: "LearningActivity 与引用的 LearningGoal owner 不一致。", path: file.path });
          if (goal.data.learning_area_id !== record.data.learning_area_id) errors.push({ code: "LEARNING_ACTIVITY_GOAL_AREA_MISMATCH", message: "LearningActivity 的 Goal 不属于所选 Area。", path: file.path });
          if (goal.deleted_at !== null || goal.data.status === "archived") warnings.push({ code: "LEARNING_ACTIVITY_PARENT_READ_ONLY", message: "LearningActivity 的 Goal 当前不可管理；历史记录保留并只读。", path: file.path });
        }
      }
      if (record.data.linked_task_id && !taskIds.has(record.data.linked_task_id)) errors.push({ code: "LEARNING_ACTIVITY_TASK_MISSING", message: "LearningActivity 引用的 Task 不在导出包中。", path: file.path });
    } catch {
      errors.push({ code: "INVALID_LEARNING_ACTIVITY_RECORD", message: "LearningActivity 文件无法通过结构校验。", path: file.path });
    }
  }
  ...
  const confirmedWorkoutStaging = new Map<string, ReturnType<typeof parseHealthStagingRecord>>();
  ...
  for (const file of healthStagingFiles) {
    try {
      ...
      if (record.data.status === "confirmed" && record.data.canonical_record_id) {
        confirmedHealthLinks.set(record.data.canonical_record_id, record.id);
        if (record.data.health_type === "workout") confirmedWorkoutStaging.set(record.data.canonical_record_id, record);
      }
  ...
  const workoutFiles = validPayloadFiles.filter((file) => file.path.startsWith("data/workouts/"));
  result.counts.workouts = workoutFiles.length;
  for (const file of workoutFiles) {
    try {
      const record = parseWorkoutRecord(file.content);
      if (result.workspace && record.owner_id !== result.workspace.owner_id) errors.push({ code: "OWNER_MISMATCH", message: "Workout 的 owner_id 与 workspace 不一致。", path: file.path });
      if (`data/workouts/${record.id}.json` !== file.path) errors.push({ code: "WORKOUT_PATH_MISMATCH", message: "Workout 的 ID 与文件路径不一致。", path: file.path });
  ...
  for (const [canonicalId, staging] of confirmedWorkoutStaging) {
    if (!workoutIds.has(canonicalId)) errors.push({ code: "WORKOUT_CANONICAL_MISSING", message: "已确认的 Workout 暂存记录缺少正式 Workout 文件。", path: `data/health-staging-records/${staging.id}.json` });
  }
