...
export async function writeLegacyJournalBatchAtomically(
  ...
  assertInstant(input.committedAt);
  const validated = await validatePlan(input.plan);
  ...
  assertRemoteState(validated.ownerId, input.plan, entries, revisions, segments, checkpoints);
  const planSha256 = await sha256Text(JSON.stringify({
    plan_version: input.plan.planVersion,
    import_batch_id: input.plan.importBatchId,
    dry_run_id: input.plan.dryRunId,
    correction_set_sha256: input.plan.correctionSetSha256,
    source_sha256: input.plan.sourceSha256,
    expected_parent_commit_sha: input.plan.expectedHeadCommitSha,
    selected_dates: input.plan.selectedDates,
    pending_items: validated.items.map((item) => ({
      date: item.date,
    ...
    planned_files: input.plan.files.map((file) => ({ path: file.path, sha256: file.sha256 })),
  }));
  const checkpointId = `journal_import_checkpoint_${planSha256.slice(0, 32)}`;
  if (checkpoints.some((record) => record.id === checkpointId)) throw new Error("LEGACY_IMPORT_CHECKPOINT_ALREADY_EXISTS");
  ...
  const checkpointRecord = createJournalImportCheckpointRecord({
    ...
    correctionSetSha256: input.plan.correctionSetSha256,
    expectedParentCommitSha: input.plan.expectedHeadCommitSha,
    planSha256,
    ...
    plannedFiles: input.plan.files.map((file) => ({ path: file.path, sha256: file.sha256 })),
  });
  const checkpointPath = recordPath("journal_import_checkpoint", checkpointRecord.id);
  const files = [
    ...input.plan.files.map((file) => ({ path: file.path, text: file.text })),
    { path: checkpointPath, text: serializeRecord(checkpointRecord) },
  ];
  if (files.length > MAX_LEGACY_JOURNAL_ATOMIC_FILES) throw new Error("LEGACY_IMPORT_ATOMIC_FILE_LIMIT_EXCEEDED");
  if (files.reduce((total, file) => total + byteLength(file.text), 0) > MAX_LEGACY_JOURNAL_ATOMIC_BYTES) throw new Error("LEGACY_IMPORT_ATOMIC_BYTE_LIMIT_EXCEEDED");

  const result = await adapter.writeAtomicFiles({
    files,
    message: `journal: import ${validated.items[0]!.date}..${validated.items.at(-1)!.date}`,
  ...
  return {
    ...
    checkpointRecord,
    checkpointFile: { path: checkpointPath, blobSha: blobShaFor(result.files, checkpointPath) },
    journalFiles,
  };
}
...
async function validatePlan(plan: LegacyJournalCommitPlan) {
  ...
  if (plan.files.length === 0 || plan.files.length !== plan.summary.files || plan.files.length >= MAX_LEGACY_JOURNAL_ATOMIC_FILES) throw new Error("LEGACY_IMPORT_PLAN_FILE_COUNT_INVALID");
